...
void main() {\
  ...
  // Multiply alpha just once to fulfill the job of premultipliedAlpha \n\
...
');
  ...
  exports.canvas = function() {
    var canvas = document.createElement('canvas');
    // premultipliedAlpha tells browser how the content drawn by WebGL should be blended with existing DOMs
    // when its value is true, the alpha multiplying is done by WebGL pipeline, so browser doesn't have to do it again.
    // premultipliedAlpha is not reliable in Chrome right now: https://bugs.chromium.org/p/chromium/issues/detail?id=599285
    // The multiply alpha is now done by `gl_FragColor.rgb *= gl_FragColor.a` in flippedShader
    var config = {premultipliedAlpha: true};
    try {
    ...
    Texture.prototype.drawTo = function(callback) {
      ...
      switch (status) {
        ...
        default:
          ...
          throw new Error("Incomplete framebuffer: " + status + " Error code: " + errorCode);
      }
